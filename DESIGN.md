...
- 사용자 프로파일(확정): 개인 1인 운영, PC/모바일 모두 자주 사용, 모바일 조회 비중 높음(동시 접속 1명 가정)
- 레퍼런스 프로젝트(동작/구조 참고):
...
- 리스트 표시 정보는 `title`, `tags`, `dateLabel` 3가지를 기준으로 한다(`favorite`은 정렬용 메타로 유지).
- 페이지네이션 메타: `page`, `pageSize`, `total`, `hasNext`
- 카드 클릭 시 상세 페이지로 이동
...
- 카드 내용: `title`, `tag`, `date`만 노출(기존 `../jwreview` 리스트 정보 수준 유지)
- 카드 전체 영역을 클릭 타겟으로 사용해 상세 페이지로 이동한다.
- 카드 밀도 권장안: 내부 여백 16px(모바일)/20px(데스크톱), 라운드 12px, 얕은 그림자 + 얇은 보더 조합
- 레퍼런스 반영: `../jwreview/dist/css/review.css`의 카드 밀도/가독성 방향 유지

### 8.2 상세(마크다운 뷰어) 화면
- GitHub 스타일 기반 타이포그래피 사용
- 제목/본문/코드블록/표/인용구 가독성 우선
- PC 최대 너비 제한: `../jwreview`와 동일한 `max-width: 768px`
...
### 8.4 마크다운 스타일 적용 방식
- 기본 채택: `github-markdown-css` 패키지 + Tailwind 레이아웃 유틸 조합
- 이유: 요구사항의 "GitHub 스타일 기반 읽기 경험" 충족을 가장 직접적으로 보장하고, 스타일 유지보수 부담을 줄인다.

### 8.5 톤/타이포그래피
- 컬러 톤은 라이트/다크 모두 차분한 중저채도 팔레트를 사용한다(과도한 대비/채도 지양).
- 폰트 권장안: `Pretendard Variable`, `Noto Sans KR`, `Apple SD Gothic Neo`, `sans-serif`
- 본문 기본 크기 권장안: 모바일 15px, 데스크톱 16px
- 카드 메타(태그/날짜)는 본문보다 한 단계 작은 크기로 계층을 분리한다.
...
### 9.4 Atlas 운영 기준
- `../memo`의 운영 기준을 그대로 따른다.
- 연결 문자열은 `.env`의 `MONGODB_URI` 단일 진입점으로 관리한다.
- 실제 값은 `../memo/.env`에 설정된 `MONGODB_URI`를 동일하게 사용한다(값 자체는 문서/코드에 기록하지 않음).
- DB명 기본값은 `jmemo`를 사용한다.
- 네트워크 허용 정책(IP Allow List)은 MongoDB Cloud에 등록 완료된 상태를 전제로 운영한다.
...
- 페이지네이션 테스트(`page`, `pageSize`, `total`, `hasNext`)
...
- 카드에 `title/tag/date`가 의도대로 표시되는지 확인
- 카드 어느 위치를 눌러도 상세로 이동하는지 확인

## 12. 구현 전 확정 체크리스트
- [x] PD-01 카테고리 필터 env 키 최종명 확정 (`JMEMO_CATEGORY_FILTER` 유지)
- [x] PD-02 상세 본문 PC 최대 너비 확정 (`../jwreview`와 동일하게 `768px`)
- [x] PD-03 Markdown 스타일 적용 방식 확정 (`github-markdown-css` 패키지)
- [x] PD-04 리스트 페이지 페이지네이션 도입 확정 (MVP 포함)
- [x] PD-05 상세 페이지 TOC(목차) 미제공 확정
- [x] PD-06 운영 Atlas 접속 기준 확정 (`../memo` 운영 기준 상속)
- [x] PD-07 대상 사용자 프로파일/사용 빈도 확정 (개인 1인 운영, PC/모바일 모두 자주 사용, 모바일 조회 비중 높음)
...
- [x] DC-08 상세 본문 최대 너비는 `../jwreview`와 동일한 `768px`로 확정한다.
- [x] DC-09 Markdown CSS 자산 공급 방식은 `github-markdown-css` 패키지로 확정한다.
- [x] DC-10 카테고리 필터 env 키 최종명은 `JMEMO_CATEGORY_FILTER`로 확정한다.
- [x] DC-11 운영 Atlas 연결값(`MONGODB_URI`, DB명, 네트워크 허용 정책)은 `../memo` 운영 기준으로 확정한다.
...
- [x] IC-09 카드 리스트 페이지는 페이지네이션을 도입하고, 기본 쿼리는 `page=1`, `pageSize=30`을 사용한다.
- [x] IC-10 상세 페이지는 TOC(목차)를 제공하지 않고 본문 가독성에 집중한다.
- [x] IC-11 카드 리스트 표시는 `title/tag/date`만 사용한다.
- [x] IC-12 카드 전체 영역을 클릭하면 상세 페이지로 이동해야 한다.
- [x] IC-13 라이트/다크 테마는 모두 차분한 톤(중저채도)으로 구성한다.
- [x] IC-14 폰트는 `Pretendard Variable` 계열 권장안으로 적용한다.
...
- DC-08: 상세 본문 최대 너비는 `../jwreview` 기준과 동일하게 `768px`로 확정한다.
- DC-09: Markdown 스타일은 커스텀 사본 대신 `github-markdown-css` 패키지를 사용한다.
- DC-10: 카테고리 필터 env 키는 `JMEMO_CATEGORY_FILTER`로 고정한다.
- DC-11: Atlas 운영 접속/네트워크 정책은 `../memo` 운영 기준을 상속한다.
- PD-07: 대상 사용자는 개인 1인 운영이며 PC와 모바일을 모두 자주 사용한다. 모바일은 조회 비중이 높고, 동시 접속은 1명으로 가정한다.
- UI-01: 리스트 카드의 표시 정보는 `title/tag/date`만 유지한다.
- UI-02: 카드 전체 영역을 클릭 가능하게 만들어 모바일 탭 사용성을 우선한다.
- UI-03: 시각 톤은 차분한 라이트/다크 팔레트를 사용한다.
- UI-04: 폰트는 `Pretendard Variable` 기반 권장안으로 확정한다.
